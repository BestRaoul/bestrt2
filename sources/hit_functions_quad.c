...
static inline void	get_quad_intersection(const ray *local_r, t_qh *data)
{
	PFPN	lt;
	vec3	local_p;
	PFPN	alpha;
...
static inline void	set_quad_values(hit_record *rec, const ray *local_r,
...
bool	hit_quad(const ray *r, const interval ray_t, hit_record *rec,
		const t_item *self)
{
	ray		local_r;
	t_qh	data;
	...
	get_quad_intersection(&local_r, &data);
	...
	rec->t = gt;
	rec->p = ray_at(r, gt);
	set_quad_values(rec, &local_r, self, &data);
	return (True);
}
